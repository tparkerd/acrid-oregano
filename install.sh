...
if [ $psql_status -ne 0 ]
  then
    ...
    if [ $psql_status -eq 3 ]
      then
        systemctl start postgresql-9.6.service || { printf "Unable to start postgresql-9.6 Aborting.\n" 1>&2; exit 1; }
    fi
    ...
    if [ $psql_status -eq 4 ]
      then
        ...
        systemctl enable postgresql-9.6.service || { printf "Unable to enable postgresql-9.6 Aborting.\n" 1>&2; exit 1; }
        systemctl start postgresql-9.6.service || { printf "Unable to start postgresql-9.6 Aborting.\n" 1>&2; exit 1; }
    fi
fi
